...
func run() {
	...
	mainthread.Call(func() {
		...
		for !win.ShouldClose() {
			...
			projection = mgl32.Perspective(mgl32.DegToRad(45), float32(screenW/ScreenH), 0.1, 100)
...
func runDraw(vao uint32, texture0 uint32, texture1 uint32, win *glfw.Window, prog uint32) {
	...
	{
		// trans1 := mgl32.Translate3D(0, 0, 0)
		trans2 := mgl32.HomogRotate3D(float32(time), mgl32.Vec3{1, 0, 0.3})
...
func makeVao(vertex []float32) uint32 {
	...
	// position
	gl.VertexAttribPointer(0, 3, gl.FLOAT, false, 5*vsize, gl.PtrOffset(0))
	gl.EnableVertexAttribArray(0)

	// texture
	gl.VertexAttribPointer(2, 2, gl.FLOAT, false, 5*vsize, gl.PtrOffset(int(3*vsize)))
